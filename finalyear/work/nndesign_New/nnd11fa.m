...
elseif strcmp(cmd,'init') & (~fig)
  ...
  H = [fig_axis desc_text s1_bar s1_text fa_axis fa_ptr i_bar i_text fb_axis];
...
elseif strcmp(cmd,'train') & (fig) & (nargin == 1)
  ...
  Target = plot(P,T,'-','color',nnblue,'linewidth',3,'erasemode','none');
  ...
  a12fine = nndlogsig(W10*Pfine+B10*ones(1,length(Pfine)));
  axes(fb_axis)
  ...
  for k=1:max_epoch,
    ...
    if (R==1)&(S2==1),
      ...
      set(Target,'color',nnltyell);
      set(Target,'color',nnblue);
      ...
      a12fine = nndlogsig(W1*Pfine+B1*ones(1,length(Pfine)));
