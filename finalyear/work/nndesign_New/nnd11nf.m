...
if fig
  ...
  tf_ptr = H(24);             % 2nd layer transfer function name
  autoscale = H(25);          % Indication for Autoscaling
end
...
elseif strcmp(cmd,'init') & (~fig)
  ...
  a = feval(cat(2,'nnd',tf),W2*nndlogsig(W1*p+b1)+b2);
  ...
  A = feval(cat(2,'nnd',tf),W2*nndlogsig(W1*P+b1*ones(1,Q))+b2*ones(1,Q));
  a_max = max(A);
  a_min = min(A);
  ...
  H = [fig_axis desc_text meters indicators  tf_icon_ptr tf_menu ...
    func_axis func_line p_line a_line value_ptr tf_ptr autoscale];
...
elseif strcmp(cmd,'f_motion') & (fig)
  ...
  % GET DATA
  mdgray = nnmdgray;
  tf = cat(2,'nnd',get(tf_ptr,'userdata'));
  values = get(values_ptr,'userdata');
  W1 = values([1 4])';
  b1 = values([2 5])';
  W2 = values([3 6]);
  ...
  % NEW INPUT & OUTPUT
  p = x;
  a = feval(tf,W2*nndlogsig(W1*p+b1)+b2);
...
if strcmp(cmd,'update') & (fig)
  ...
  if get(autoscale,'value')==1
     ...
     set(p_line,'color',red,'xdata',[p p],'ydata',[-6 6])
  end
  hold on
